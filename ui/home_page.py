"""
Главная страница - парсинг документов
"""
...
💡 Tip: Pin your BOM and Manager files once - they'll stay for future use!"""
    ...
    def on_parsing_complete(self, result, pdf_path):
        ...
        # Создаем словарь для быстрого доступа к table3
        info_dict = {}
        for item in table3:
        ...
        tag_no = info_dict.get("TAG No", None)
        customer = info_dict.get("CUSTOMER", None)
        project = info_dict.get("PROJECT/LOCATION", None)
        pdf_name = os.path.basename(pdf_path)
        
        # Считаем статистику
        stats = self.calculate_stats(table2)
        
        # Добавляем в историю с customer и project
        self.history_manager.add_record(pdf_name, tag_no, customer, project, stats)
